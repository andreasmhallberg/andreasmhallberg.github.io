...
\newcommand{\fix}[2]{%
  ...
  \begin{tikzpicture}
    ...
    \draw[draw=white, fill=\fixcolor] ($ (fix.south west)!0.5!(fix.south east) $) ++ (0,.5ex) circle (2pt);
  \end{tikzpicture}%
  \tikzmark{#2b}%
}
...
\newcommand{\regfix}[2]{%
  ...
  \begin{tikzpicture}
    \node [inner sep = 0pt, text depth=0pt](fix) {#1};
    \draw[draw=\fixcolor, fill=\fixcolor, transform canvas={yshift=-1.5ex}]
...
\begin{tikzpicture}[remember picture]
  \draw[->,>=latex, overlay, draw=\saccolor, shorten < =1pt, shorten > =1pt, transform canvas={yshift=-2.5ex}]
    ($(pic cs:#1) + (0,.5ex)$) --
    ($(pic cs:#2) + (0,.5ex)$);
\end{tikzpicture}%
}
...
\newcommand{\fixinfo}[3][1]{%
  ...
  \node[infoboxfix, yshift=#1\infosep] (infobox) at (#2) {\begin{varwidth}{3cm}\center\color{\fixcolor}#3\end{varwidth}} ;
...
\node[infoboxfix, yshift=#1\infosep] (infobox) at (pic cs:#2) {\begin{varwidth}{3cm}\center\color{\fixcolor}#3\end{varwidth}} ;
...
\node[infoboxsac, yshift=#1\infosep] (infobox) at (sac) {\begin{varwidth}{3cm}\center\color{\saccolor}#4\end{varwidth}} ;
...
\ttfamily\huge
  ...
  This ex\fix{a}{fix1}mple ill\fix{u}{fix2}stra\fix{t}{fix3}es e\tikzmark{shortword}ye mo\fix{v}{fix4}em\regfix{e}{regfix}nts ty\fix{p}{fix5}ical o\tikzmark{function}f re\fix{a}{fix6}ding.%
  \tikzmark{sentensend}%
...
\begin{tikzpicture}[remember picture, overlay, inner sep=0pt]
  ...
  \fixinfo[1.7]{fix2}{Words of 8 letters or more are almost always fixated.}

  \fixinfo[1.7]{fix1}{Fixations are 200--250~ms. long.}
  ...
  \sacinfo{fix5}{regfix}{10-15\% of all saccades move backwards. They most often target the previous word.}

  \fixinfo[1.7]{fix6}{Fixation times are longer at the end of a clause or sentence.}
